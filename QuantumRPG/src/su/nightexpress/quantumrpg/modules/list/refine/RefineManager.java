...
public class RefineManager extends QModuleDrop<RefineItem> {
	...
	private int silentRateBonusCap = 20; // TODO INIT
	...
	public boolean canRefine(@NotNull ItemStack item) {
		...
		if (!ItemUtils.isArmor(item) && !ItemUtils.isWeapon(item)) return false;
	 ...
	 * @param lvl Enchantment Stone level
	 * @return
	 */
	@NotNull
	...
	private void addFines(@NotNull ItemStack item, @NotNull RefineItem stone, int refineLvl) {
		...
		ItemUT.addNameTag(item, TAG_REFINE_NAME, name);
		ItemUT.addLoreTag(item, TAG_REFINE_LORE, loreTag.toString());
		
		// Replace old stat lore tags with refined ones.
		statTags.forEach((stat, value) -> {
			ItemUT.addLoreTag(item, stat.getMetaId(item), value);
		});
	...
	public class RefineItem extends RatedItem {
		...
		private TreeMap<Integer, BonusMap> refineBonusMap;
