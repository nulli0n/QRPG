...
public class AnvilGUI extends NGUI<QuantumRPG> {
	...
	public AnvilGUI(@NotNull RepairManager repairManager) {
		...
		GuiClick clickMain = (p, type, e) -> {
			if (type == ContentType.EXIT) {
				...
				RepairType rerpairType = getSelectedType(inv);
		...
		for (String itemId : cfg.getSection(path + "repair-types")) {
			GuiItem guiItem = cfg.getGuiItem(path + "repair-types." + itemId, RepairType.class);
			if (guiItem == null) continue;
			
			Enum<?> type = guiItem.getType();
			if (type != null && type.getClass().equals(RepairType.class)) {
				RepairType repairType = (RepairType) type;
	...
	public void open(
		...
		for (GuiItem guiItem : new HashSet<>(this.getContent().values())) {
			...
			if (type2 == null || !type2.getClass().equals(RepairType.class)) {
				continue;
			}
			RepairType repairType = (RepairType) type2;
			...
			GuiClick clickRepair = new GuiClick() {
				...
				public void click(
					...
					open(p, target, src, (RepairType) type2);
				}
			};
	...
	@Nullable
	private RepairType getSelectedType(@NotNull Inventory inv) {
		for (GuiItem guiItem : this.getContent().values()) {
			Enum<?> type2 = guiItem.getType();
			if (type2 == null || !type2.getClass().equals(RepairType.class)) continue;
