...
public class DamageManager extends IListener<QuantumRPG> {
	...
	private boolean handleDamageModifiers(
		...
		double directProcent = event.getDamagerItemStat(AbstractStat.Type.DIRECT_DAMAGE) / 100D;
		double penet = 1D - event.getDamagerItemStat(AbstractStat.Type.PENETRATION) / 100D;
		...
		meta.setDirectModifier(directProcent);
		meta.setPenetrateModifier(penet);
