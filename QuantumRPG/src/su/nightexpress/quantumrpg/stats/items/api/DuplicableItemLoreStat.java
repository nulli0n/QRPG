...
public abstract class DuplicableItemLoreStat<Z> extends ItemLoreStat<Z> {
	...
	private Map<Integer, NamespacedKey> keys;
	...
	public final int getAmount(@NotNull ItemStack item) {
    	...
    	ItemMeta meta = item.getItemMeta(); if (meta == null) return value;
    	List<String> lore = meta.getLore(); if (lore == null) return value;
		
    	PersistentDataContainer container = meta.getPersistentDataContainer();
    	
		for (int index = 0; index < lore.size(); index++) {
			if (container.has(this.getKey(index) , this.dataType)) value++;
		}
		return value;
	}
	...
	public final int getLoreIndex(@NotNull ItemStack item, int index) {
    	...
    	ItemMeta meta = item.getItemMeta(); if (meta == null) return -1;
    	List<String> lore = meta.getLore(); if (lore == null) return -1;
